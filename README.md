...
- I realized that in order to reach the target accuracy of 75% or higher we may need to go back and do the preprocess steps a bit more diligently, study what input variables is truly needed, and how it should be transformed before optimizing the models. I am sure they may be a better solution to preprocess the dataset, however, I was satisfied with being just 3% short of target prediction. It is a small variance, and 72% accuracy is sufficient to assume that the model's predictions to identify which organizations are worth the donations and which ones are high risks is accurate.
